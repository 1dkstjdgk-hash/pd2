...
[실행 시간]
  ...
  재실행:    ~1분 이내 (.bt_cache_v12/ 청크 캐시 자동 재사용)
  ※ v12는 .bt_cache_v12/ 을 전용으로 사용 → v11 캐시와 격리
...
[v10 버그 수정 — 2026-03-08]
  ...
  BUG-4  캐시 버전 분리: .bt_cache → .bt_cache_v11 (v10 캐시 재사용 시 변경 무효화 방지)
...
class MultipleTestingGuard:
    ...
    def combined_test(self, t_stats: dict, n_obs: int) -> dict:
        ...
        # 양측 p값: p = 2 × P(T > |t|)
        p_arr  = [float(2 * _s.t.sf(abs(t), df=max(n_obs - 1, 1))) for t in t_arr]
...
class TransactionCostEngine:
    ...
    # 거래량 기반 슬리피지 테이블 (단방향 %)
    _SLIPPAGE = {
        "large":  (5_000_000,  0.05),   # >5M주/일
        "mid":    (1_000_000,  0.10),   # 1~5M주/일
        "small":  (300_000,    0.20),   # 0.3~1M주/일
        "micro":  (0,          0.50),   # <0.3M주/일
    }
    ...
    def estimate(self, price: float, avg_daily_volume: float,
        ...
        """
        # 슬리피지 계산 (Kissell 2013)
        adv = max(float(avg_daily_volume), 1.0)
        if adv >= 5_000_000:
            slip_pct = 0.05
        elif adv >= 1_000_000:
            slip_pct = 0.10
        ...
        daily_turnover = float(price) * adv
        ...
        for fold in folds:
            ...
            for col in raw_cols:
                ...
                for _, grp in tr_df.groupby("date"):
                    ...
                    if len(valid) < 4:
                        continue
                    ic, _ = _sp(valid[col].values, valid[fwd_col].values)
                    if not np.isnan(ic):
    ...
    def analyze_by_regime(self, df: pd.DataFrame,
        ...
        for regime in ["bull", "sideways", "bear"]:
            ...
            for _, grp in sub.groupby("date"):
                ...
                if len(_rv) < 4:
                    continue
                ic, _ = _sp(_rv[score_col].values.astype(float),
...
def run_backtest_single(tk: str, df: pd.DataFrame, spy_df: pd.DataFrame,
    ...
    # ③ TransactionCostEngine: 전체 기간 일평균 거래량
    ...
    for i in range(LOOKBACK, n - max(FORWARD)):
        try:
            ...
            # ③ raw_iv_proxy: 내재변동성 근사 (ATR/HV 비율)
            #    실제 옵션 IV 없을 때 대체 — IV/RV 비율의 근사
            #    ATR(14) / HV(20) ≈ 시장의 단기 불확실성 vs 실현변동성
            try:
                ...
                raw_iv_proxy = (_atr14 / max(_hv20, 1e-9)
                                if (np.isfinite(_atr14) and np.isfinite(_hv20) and _hv20 > 0)
                                else np.nan)
                raw_iv_proxy = _safe_raw(raw_iv_proxy)
            except Exception:
                raw_iv_proxy = float("nan")
            ...
            record = {
                ...
                "raw_iv_proxy": raw_iv_proxy,  # IV/RV 근사 비율
...
def _ic_optimal_weights(df: pd.DataFrame,
    ...
    과적합 방지:
      훈련/검증 기간을 완전 분리 (날짜 기준 50:50)
      가중치는 훈련 기간 IC만으로 결정
      검증 기간 IC는 이 함수에서 절대 사용하지 않음
    """
    from scipy.stats import spearmanr as _sp
...
def cross_sectional_rank(df: pd.DataFrame) -> pd.DataFrame:
    ...
    #   단기: vol_z + bb_z + rs + iv_proxy(ATR/HV, 내재변동성 근사)
    ...
    # ─────────────────────────────────────────────────────────
    raw_st_t1 = ["raw_vol_z", "raw_bb_z", "raw_rs", "raw_iv_proxy"]  # TIER1 단기
    ...
    for date, grp in df.groupby("date"):
        ...
        for tier_val in [1, 2, 3]:
            ...
            #
            # 근거: Jegadeesh & Titman (2001) JF — "소형주의 단기 과잉반응
            #         후 mean reversion은 well-documented된 현상"
            #       Baker & Stein (2004) JFE — "유동성 낮은 종목일수록
            #         과매수/과매도 후 반전 강도 높음"
            #       실증: TIER3 하위20% IC=-0.04(역방향) ← 신호가 역전을 맞춤
            # ══════════════════════════════════════════════════════════
            if tier_val == 3:
...
def analyze(df: pd.DataFrame) -> dict:
            ...
            for fwd in FORWARD:
                ...
                if sh_col in sub.columns:
                    ...
                    if len(sh) >= 10:
                        ...
                        # 분석 단계에서 클리핑 → 캐시(.bt_cache_v12) 삭제 불필요
    ...
    # 2. 서브컴포넌트 상관관계
    # 단기 원점수 상관계수 — v5 재구성
    sub_cols = ["raw_vol_z","raw_bb_z","raw_rs","raw_accel"]
    sub_labels = {"raw_vol_z": "거래량 서지(vol_surge)",
                  "raw_bb_z":  "BB압축 역수",
                  "raw_rs":    "RS vs SPY 5일",
                  "raw_accel": "가격 가속도(5d-20d/4)"}
    # 장기 원점수 상관계수 — v5 재구성
    ...
    for score_col, label in [("cs_score","CS단기"), ("cf_score","CS장기"),
        ...
        for _d, _g in df.groupby("date"):
            ...
            for ret_col, ic_list in [("ret_20d", ic_20d), ("ret_60d", ic_60d)]:
                ...
                ic, _ = _stats.spearmanr(_valid[score_col].values, _valid[ret_col].values)
    ...
    for _boot_idx, (score_col, label) in enumerate(
        ...
        for _d, _g in df.groupby("date"):
            ...
            if len(_valid) < 4: continue
            ic, _ = _sp2(_valid[score_col].values.astype(float),
...
def main():
    ...
    # v12: 신규 팩터(dvol/mfi/iv_proxy/sharpe) → v11 캐시와 격리 필수
    CACHE_DIR   = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".bt_cache_v12")
    ...
    for _rc in ["raw_vol_z","raw_bb_z","raw_rs","raw_accel",
        ...
        _all_c  = (_s.std() < 1e-9) if len(_s) > 0 else True
        _status = "❌ 전체상수(NaN위험)" if _all_c else ("⚠️ NaN많음" if _n_nan > len(_df_diag)*0.3 else "✅ 정상")
        print(f"    {_rc:16s}: {_status}  유효N={len(_s):,}  NaN={_n_nan:,}  std={_s.std():.4f}")
    ...
    for tier_name, tier_set, is_tier3 in [
        ...
        # ALERT-2: TIER3는 역발상(100-cs_combo) 방향으로 IC 계산
        # → "저점수 종목을 매수"하는 역발상 전략의 실제 IC
        if is_tier3 and "cs_combo" in sub.columns:
            sub = sub.copy()
            sub["cs_combo"] = 100.0 - sub["cs_combo"]
            tier3_note = " [역발상 적용: 100-cs_combo]"
        else:
            tier3_note = ""
        ...
        for _d, _g in sub.groupby("date"):
            ...
            valid = _g[["cs_combo","ret_20d"]].dropna()
            if len(valid) < 3: continue
            sc  = valid["cs_combo"].values
            r20 = valid["ret_20d"].values.astype(float)
            ic_val, _ = _stats2.spearmanr(sc, r20)
            if not np.isnan(ic_val): ic_arr.append(ic_val)
            if "ret_20d_xs" in _g.columns:
                valid_xs = _g[["cs_combo","ret_20d_xs"]].dropna()
                if len(valid_xs) >= 3:
                    ic_xs, _ = _stats2.spearmanr(valid_xs["cs_combo"].values,
    ...
    for _tname, _tset, _is3 in [
        ...
        for _, _g in _sub.groupby("date"):
            ...
            _ic, _ = _sr2(_v["cs_combo"].values, _v["ret_20d"].values)
