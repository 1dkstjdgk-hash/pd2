...
def calc_rebalance(target: pd.Series, existing: Dict[str, int], prices: pd.Series) -> pd.DataFrame:
    ...
    for tk in all_tk:
        ...
        n = abs(delta)
        comm = max(n*COMM_PER_SHARE, MIN_COMM) if n>0 else 0.0
        slip = n * px * SLIPPAGE_PCT if n>0 else 0.0
        rows.append({"ticker":tk, "현재수량":cur, "목표수량":tgt, "매매수량":delta,
                     "action":action, "현재가":round(px,2), "거래금액":round(n*px,2),
                     "수수료+슬리피지":round(comm+slip,2), "비중괴리":round(drift*100,1)})
...
body { font-family: 'Segoe UI', Consolas, sans-serif; background: #0d1117; color: #c9d1d9; padding: 28px; }
.wrap { max-width: 1360px; margin: 0 auto; }
h1 { color: #58a6ff; text-align: center; font-size: 26px; margin-bottom: 4px; letter-spacing: -0.5px; }
.sub { text-align: center; color: #8b949e; font-size: 11px; margin-bottom: 6px; }
.nav { display: flex; gap: 8px; justify-content: center; margin-bottom: 28px; flex-wrap: wrap; }
.nav a { padding: 6px 18px; border-radius: 20px; background: #161b22; border: 1px solid #30363d;
...
.card { flex: 1; min-width: 110px; background: #161b22; border: 1px solid #30363d;
        border-radius: 8px; padding: 13px; text-align: center; }
.card .cl { font-size: 10px; color: #8b949e; text-transform: uppercase; margin-bottom: 5px; letter-spacing: .4px; }
.card .cv { font-size: 18px; font-weight: 700; }
section { margin-bottom: 40px; padding-top: 12px; }
h2 { color: #58a6ff; font-size: 13px; margin: 0 0 10px;
...
th { color: #8b949e; font-size: 10px; text-transform: uppercase;
     padding: 6px 8px; text-align: right; border-bottom: 2px solid #30363d; white-space: nowrap; }
th:first-child { text-align: left; }
td { padding: 7px 8px; text-align: right; border-bottom: 1px solid #21262d; vertical-align: middle; }
td:first-child { text-align: left; }
tr:hover { background: #1c2128; }
.ticker { font-weight: 700; color: #58a6ff; font-size: 14px; }
.bold { font-weight: 700; }
.green { color: #3fb950; } .red { color: #f85149; }
.yellow { color: #d29922; } .purple { color: #d2a8ff; }
.dim { color: #8b949e; font-size: 11px; }
small { font-size: 11px; }
.note { font-size: 11px; color: #8b949e; margin: 4px 0 14px; line-height: 1.6; }
.rgrid { display: grid; grid-template-columns: repeat(5,1fr); gap: 10px; margin-bottom: 14px; }
.rcard { background: #161b22; border: 1px solid #30363d; border-radius: 7px; padding: 12px; text-align: center; }
.rcard .rl { font-size: 10px; color: #8b949e; margin-bottom: 4px; text-transform: uppercase; }
.rcard .rv { font-size: 16px; font-weight: 700; }
.tax-grid { display: grid; grid-template-columns: repeat(5,1fr); gap: 10px; margin-bottom: 12px; }
.tax-card { background: #161b22; border: 1px solid #30363d; border-radius: 7px; padding: 12px; }
.tax-card .tl { font-size: 10px; color: #8b949e; text-transform: uppercase; margin-bottom: 5px; }
.tax-card .tv { font-size: 16px; font-weight: 700; }
.alert-sl { background: rgba(248,81,73,.10); } .alert-tp { background: rgba(63,185,80,.10); }
.badge { display:inline-block; padding:2px 7px; border-radius:10px; font-size:10px; font-weight:700; }
.badge-sl { background:rgba(248,81,73,.2); color:#f85149; }
.badge-tp { background:rgba(63,185,80,.2); color:#3fb950; }
.badge-lt { background:rgba(63,185,80,.15); color:#3fb950; }
.badge-st { background:rgba(248,81,73,.15); color:#f85149; }
.signal { display:inline-block; padding:3px 10px; border-radius:12px; font-weight:700; font-size:12px; }
.sig-buy { background:rgba(63,185,80,.2); color:#3fb950; }
.sig-sell { background:rgba(248,81,73,.2); color:#f85149; }
.sig-hold { background:rgba(210,153,34,.2); color:#d29922; }
.score-bar { width:100%; background:#21262d; border-radius:4px; height:5px; margin-top:3px; }
.score-fill { height:5px; border-radius:4px; }
.news-line { font-size:10px; color:#8b949e; margin:1px 0; }
.check-box { background:#161b22; border:1px solid #30363d; border-radius:8px; padding:14px; }
.check-item { padding:5px 0; font-size:12px; border-bottom:1px solid #21262d; }
.check-item:last-child { border-bottom:none; }
.chk { color:#3fb950; margin-right:7px; }
"""
...
<p class="note">리밸런싱 임계: ±{REBAL_THRESHOLD*100:.0f}%p 미만 → HOLD | IBKR ${COMM_PER_SHARE}/주 | 슬리피지 {SLIPPAGE_PCT*100:.2f}%</p>
...
def run_commander() -> Optional[str]:
    ...
    rebal_df = calc_rebalance(shares, existing, prices_now) if existing else None
    ...
    if rebal_df is not None:
        ...
        tcost = sum(max(int(shares.get(t,0))*COMM_PER_SHARE, MIN_COMM)
                    + int(shares.get(t,0))*float(prices_now.get(t,0))*SLIPPAGE_PCT for t in active)
