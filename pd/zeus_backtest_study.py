...
# 초보자: 이 함수 '_find_dashboard'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# ① LookAheadGuard
# ──────────────────────────────────────────────────────────────────────
# 초보자: 이 클래스 'LookAheadGuard'은 관련 데이터를 묶고 메서드 동작을 정의합니다.
class LookAheadGuard:
    ...
    """
    # 초보자: 이 함수 '__init__'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    def __init__(self):
    ...
    # 초보자: 이 함수 'regime_at'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'validate_slice'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'summary'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# ──────────────────────────────────────────────────────────────────────
# 초보자: 이 클래스 'MultipleTestingGuard'은 관련 데이터를 묶고 메서드 동작을 정의합니다.
class MultipleTestingGuard:
    ...
    # 초보자: 이 함수 'bh_fdr'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'bonferroni'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'combined_test'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    @staticmethod
    # 초보자: 이 함수 'false_positive_risk'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# ──────────────────────────────────────────────────────────────────────
# 초보자: 이 클래스 'TransactionCostEngine'은 관련 데이터를 묶고 메서드 동작을 정의합니다.
class TransactionCostEngine:
    ...
    # 초보자: 이 함수 'estimate'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'adjust_returns'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'portfolio_annual_drag'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# ──────────────────────────────────────────────────────────────────────
# 초보자: 이 클래스 'RollForwardValidator'은 관련 데이터를 묶고 메서드 동작을 정의합니다.
class RollForwardValidator:
    ...
    # 초보자: 이 함수 '_make_folds'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'run'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'stability_score'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# ⑤ BootstrapCI
# ──────────────────────────────────────────────────────────────────────
# 초보자: 이 클래스 'BootstrapCI'은 관련 데이터를 묶고 메서드 동작을 정의합니다.
class BootstrapCI:
    """
    Bootstrap 신뢰구간 모듈.
    ...
    # 초보자: 이 함수 '_block_bootstrap_sample'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'ic_ci'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'sharpe_ci'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    def sharpe_ci(self, returns: np.ndarray,
        ...
        # 초보자: 이 함수 '_sr'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
        def _sr(a):
            mu  = a.mean()
            sig = a.std()
            if sig < 1e-9:
    ...
    # 초보자: 이 함수 'mean_return_ci'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# ⑥ RegimeAnalyzer
# ──────────────────────────────────────────────────────────────────────
# 초보자: 이 클래스 'RegimeAnalyzer'은 관련 데이터를 묶고 메서드 동작을 정의합니다.
class RegimeAnalyzer:
    ...
    # 초보자: 이 함수 'label_dates'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'analyze_by_regime'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# ─────────────────────────────────────────────────────────────
# 초보자: 이 함수 'download_ticker'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# 초보자: 이 함수 '_safe_raw'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# 초보자: 이 함수 'compute_spy_ret'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# 초보자: 이 함수 'run_backtest_single'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# 초보자: 이 함수 'score_bucket'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# 초보자: 이 함수 '_ic_optimal_weights'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# 초보자: 이 함수 'cross_sectional_rank'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
def cross_sectional_rank(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # (각 점수의 예측력에 비례해 결합)
    # 초보자: 이 함수 '_score_ic'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'pct_rank_grp'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'rerank'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    def rerank(arr):
        n = len(arr)
    ...
    for date, grp in df.groupby("date"):
        ...
        for tier_val in [1, 2, 3]:
            ...
            if tier_val == 1:
                # 전략A: 기관 주문흐름 팩터 포함
                # BUG-6 수정: 컬럼 존재 여부 확인 후 해당 인덱스의 가중치만 추출
                # (슬라이싱 :len(_cols)은 순서 가정 → 컬럼 누락 시 가중치 어긋남)
                _all_st = raw_st_t1
                _all_lt = raw_lt_t1
                _full_w_st = w_st_t1
                _full_w_lt = w_lt_t1
            else:
                # 전략B: 기존 팩터 (IC는 샤프 기준으로 훈련됨)
                _all_st = raw_st_t23
                _all_lt = raw_lt_t23
                _full_w_st = w_st_t23
                _full_w_lt = w_lt_t23
...
# 초보자: 이 함수 'cs_bucket'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# 초보자: 이 함수 'analyze'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
def analyze(df: pd.DataFrame) -> dict:
    ...
    # 초보자: 이 함수 '_quantile_bucket'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 '_abs_bucket_v11'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'make_bucket_stats'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    # 초보자: 이 함수 'make_sharpe_bucket_stats'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    ...
    ic_results = {}
    for score_col, label in [("cs_score","CS단기"), ("cf_score","CS장기"),
                              ("cs_combo","CS종합"), ("score","절대SSE(구버전)"),
        ...
        # 초보자: 이 함수 '_stat'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# 초보자: 이 함수 'compute_top_bottom'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
...
# 초보자: 이 함수 'build_html'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
def build_html(results: dict, elapsed: float) -> str:
    ...
    # ── 구간별 테이블 행 생성 ──
    # 초보자: 이 함수 'color_ret'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    def color_ret(v):
        if v is None: return "#666"
        return "#00ff88" if v > 0.5 else "#ff4466" if v < -0.5 else "#ffd700"
    # 초보자: 이 함수 'color_corr'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    def color_corr(v):
        if v is None: return "#666"
...
# ─────────────────────────────────────────────────────────────
# 초보자: 이 함수 'main'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
def main():
    t0 = time.time()
    print(f"\n{'='*55}")
    print(f"  SmartScore 백테스트  |  {len(TICKERS)}종목  |  기간:{PERIOD}")
    print(f"{'='*55}")
    ...
    try:
        ...
        if avail_gb < 3.0:
            ...
            print(f"      zeus_backtest_v11.py 상단 PERIOD = '5y' 로 수정 후 재실행")
        elif avail_gb < 5.0:
            print(f"  ⚠️  메모리 여유 5GB 미만 → 청크 단위 처리로 자동 대응")
    except ImportError:
        print("  💾 메모리 확인: pip install psutil 로 설치 시 자동 체크")

    # 1. 데이터 다운로드
    print("\n[1/3] 데이터 다운로드...")
    print(f"  ⏱  예상 소요: ~{len(TICKERS)//30}분 (다운로드) + ~{len(TICKERS)//10}분 (백테스트)")
    print(f"  📦 총 {len(TICKERS)}종목 | 계층1:{len(TIER1_SET)} 계층2:{len(TIER2_SET)} 계층3:{len(TIER3_SET)}")
    data = {}
    with ThreadPoolExecutor(max_workers=4) as pool:
        # 484종목: 4워커 = yfinance rate limit 실증적 안전값
        # 8→4로 하향: rate limit 경험자 환경 기준
        # 다운로드 시간 약 15~25분 (재시도 없을 경우)
        futs = {pool.submit(download_ticker, tk): tk for tk in TICKERS}
    ...
    spy_df = data.get("SPY")
    print(f"\n  다운로드 성공: {len(data)}/{len(TICKERS)}개")

    # ── ① 시장 국면 감지 (v6: LookAheadGuard 사용)
    # [기존] detect_regime(_spy_c) → 전체 SPY로 단 한 번 계산 → 룩어헤드 바이어스
    # [v6]   _lag_guard.regime_at()가 run_backtest_single 내부에서
    #         시점별로 iloc[:i+1] 슬라이싱 후 국면 계산
    #         여기서는 마지막 시점 국면만 콘솔 출력용으로 계산
    _global_regime = "sideways"
    if spy_df is not None and len(spy_df) >= 200:
        try:
            _spy_c = pd.to_numeric(spy_df["Close"], errors="coerce").dropna()
            # 콘솔 출력용으로만 사용 — 백테스트 루프는 LookAheadGuard 사용
            ...
            print(f"  ℹ️  백테스트 루프 내 각 시점별 국면은 LookAheadGuard가 처리")
        except Exception:
            pass

    # 2. 롤링 백테스트 — 청크 단위 처리 (메모리 8GB 이하 환경 대응)
    import gc, os
    print("\n[2/3] 롤링 SmartScore 계산... (종목별 병렬, 청크 단위)")

    # 원시 가격 데이터 dict → spy_df 제외 후 사용, 이후 gc
    items = list(data.items())
    del data  # 원시 다운로드 데이터 즉시 해제 (메모리 확보)
    gc.collect()

    CHUNK_SIZE  = 50
    chunks      = [items[i:i+CHUNK_SIZE] for i in range(0, len(items), CHUNK_SIZE)]
    # BUG-4 수정: 버전별 캐시 디렉터리 분리
    # v10은 _safe_raw() 저장 방식이 v8/v9와 다름 → 구 캐시 재사용 시 FIX-1 무효화
    # v11: 팩터 선별(accel/w52/cons 제거) → cross_sectional_rank 변경 → v10 캐시 무효
    # v12: 신규 팩터(dvol/mfi/rv_ratio/sharpe) → v11 캐시와 격리 필수
    # v12b: raw_iv_proxy → raw_rv_ratio 명칭 변경 → v12 캐시 무효화
    ...
    # 초보자: 이 함수 '_run_one'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    def _run_one(args):
        tk, df = args
    ...
    for ci, chunk in enumerate(chunks):
        cache_path = os.path.join(CACHE_DIR, f"chunk_{ci:03d}.pkl")

        # 이미 계산된 청크는 스킵 (재실행 시 이어받기)
    ...
    print("\n  📦 청크 병합 중...")
    # ══════════════════════════════════════════════════════════
    # [ISSUE-3] 메모리 최적화 — 청크 스트리밍 병합 + dtype 다운캐스팅
    # ══════════════════════════════════════════════════════════
    # 문제: pd.concat([read_pickle(f) for all f]) 는 모든 청크를
    #       동시에 RAM에 적재 → 8GB 이하 환경에서 OOM 위험
    # 해결:
    #   1) 청크를 한 번에 하나씩 읽어 concat (스트리밍)
    #   2) float64 → float32 다운캐스팅 (수익률 컬럼: 소수2자리면 충분)
    #      메모리 절감: ~40% (float64→32 절반)
    #   3) 병합 후 즉시 gc.collect()
    # 근거: pandas 공식 docs "Scaling to large datasets"
    #       float32 정밀도: ±1e-7 → 수익률 0.01% 단위 정밀도 유지
    # ══════════════════════════════════════════════════════════
    _chunk_dfs = []
    for _cf in chunk_files:
        _cdf = pd.read_pickle(_cf)
        # float64 → float32 다운캐스팅 (수치 컬럼만)
    ...
    out_path = "/mnt/user-data/outputs/backtest_smartscore.html"
    ...
    # 초보자: 이 함수 '_bar'은 아래 로직을 실행합니다. 입력값을 처리하고 출력을 반환합니다.
    def _bar(v):
        import math
    ...
    for _rc in ["raw_vol_z","raw_bb_z","raw_rs","raw_accel",
        ...
        _all_c  = (_scipy_stats.std() < 1e-9) if len(_s) > 0 else True
        _status = "❌ 전체상수(NaN위험)" if _all_c else ("⚠️ NaN많음" if _n_nan > len(_df_diag)*0.3 else "✅ 정상")
        print(f"    {_rc:16s}: {_status}  유효N={len(_s):,}  NaN={_n_nan:,}  std={_scipy_stats.std():.4f}")
