...
# -*- coding: utf-8 -*-
"""
ZEUS UNIFIED TRADING SYSTEM  v2.0
...
실행:
    python zeus_unified.py --server        HTTP 서버 (포트 8765) ★권장
    python zeus_unified.py --server --live  + 5분 자동갱신
    python zeus_unified.py                  HTML 파일만 생성
"""
from __future__ import annotations

import builtins, importlib.util, io, json, os, re, subprocess
...
OUT_DASH    = HERE / "zeus_out_dashboard.html"
OUT_AN      = HERE / "zeus_out_analyzer.html"
OUT_BT      = HERE / "zeus_out_backtest.html"
OUT_TRD     = HERE / "zeus_out_trading.html"
UNIFIED     = HERE / "zeus_unified.html"
...
def build_unified_html():
    ...
    dash_src=OUT_DASH.name if OUT_DASH.exists() else "about:blank"
...
<title>ZEUS Trading System</title>{_CSS}</head><body>
<div id="hdr">
  ...
  <div style="display:flex;align-items:center;gap:10px;flex-shrink:0">
    ...
    <button class="btn pri" id="rfbtn" onclick="refreshDash()">갱신</button>
  </div>
</div>
<div id="lnk">
...
<div class="zpnl" id="p0">
  <div id="dash-log" class="logbox" style="margin:10px 20px 0"></div>
  <iframe class="zframe" id="dash-frm" src="{dash_src}"></iframe>
</div>
...
<div class="zpnl" id="p1">
  ...
  <div style="position:sticky;top:0;z-index:100;background:var(--bg);border-bottom:1px solid var(--border);padding:12px 20px;display:flex;flex-wrap:wrap;align-items:flex-end;gap:12px">
    <div style="flex:2;min-width:200px">
      <label style="font-size:.75rem;color:var(--muted);display:block;margin-bottom:4px">티커 (콤마 구분)</label>
      <input id="an-tk" type="text" placeholder="NVDA, AAPL, META, TSLA"
    ...
    <div style="display:flex;flex-direction:column;gap:5px">
      <button class="btn pri" id="an-btn" onclick="runAnalyzer()" style="white-space:nowrap">분석 실행</button>
    </div>
  </div>
  <!-- 진행 바 + 로그 (컴팩트) -->
  ...
  <div id="an-inject" class="an-inject"></div>
</div>
...
<div class="zpnl" id="p2">
  <div class="panel-wrap">
    <div class="card">
      ...
      <div class="acts">
        <button class="btn pri" id="bt-btn" onclick="runBacktest()">백테스트 실행</button>
  ...
  <div class="card" id="sig-card" style="margin:0 20px 16px;{bt_display}">
    ...
    <div style="margin-top:10px;font-size:.8rem;color:var(--muted)">
      Trading Commander에 자동 로드됩니다.
      <button class="btn ok" style="padding:2px 9px;font-size:.75rem;margin-left:8px" onclick="sigToTrading()">Trading으로 이동</button>
    </div>
  </div>
  <div class="res" id="bt-res" style="padding:0 20px 20px"><iframe class="resfrm" id="bt-frm" style="min-height:700px"></iframe></div>
</div>
...
<div class="zpnl" id="p3">
  <div class="panel-wrap"><div class="card">
    ...
    <div class="acts">
      <button class="btn pri" id="sc-btn" onclick="runScreener()">스크리닝 실행</button>
  ...
  <div class="card" id="sc-card" style="margin:0 20px 16px;display:none">
    <h2>🎯 통과 종목 &nbsp;<span id="sc-count" style="color:var(--green);font-weight:700"></span></h2>
    <div class="chips" id="sc-chips"></div>
    <div style="margin-top:10px;font-size:.8rem;color:var(--muted)">
      급락 후 응축 중인 종목입니다.
      <button class="btn ok" style="padding:2px 9px;font-size:.75rem;margin-left:8px"
        onclick="scToTrading()">Trading으로 이동</button>
    </div>
  </div>
</div>
...
<div class="zpnl" id="p4">
  {"<div style='margin:14px 20px;padding:9px 14px;border-radius:8px;border:1px solid "+rg_col+";font-size:.84rem'>국면: <b style='color:"+rg_col+"'>"+rg_lbl+"</b>  F&G: <b>"+str(fg)+"</b>  → max_weight <b style='color:var(--yellow)'>"+str(int(max_w*100))+"%</b> 자동 적용</div>" if rg else ""}
  <div class="tgrid">
    <div class="card" style="margin:0">
      <h2>Position Commander</h2>
      <p>포지션 사이징 (ERC / Kelly / EW)<br>Backtest 신호 자동 로드 · 국면별 max_weight 자동 조정</p>
      <div class="frow"><label>티커 (비우면 Backtest 신호 자동 로드)</label>
        <input id="cmd-tk" type="text" value="{cmd_tk_val}" placeholder="NVDA, META, AMZN  또는  공백"></div>
      <div class="fgrid">
        <div class="frow"><label>투자금액 (USD)</label><input id="cmd-cap" type="number" value="100000" min="1000"></div>
        <div class="frow"><label>전략</label>
          <select id="cmd-st"><option value="1" selected>Risk Parity (ERC)</option><option value="2">Half-Kelly</option><option value="3">Equal Weight</option></select></div>
      </div>
      <div class="acts">
        <button class="btn pri" id="cmd-btn" onclick="runCmd()">포지션 계산</button>
        <button class="btn" onclick="loadSigTk()">신호 로드</button>
      </div>
      <div class="logbox" id="cmd-log"></div>
    </div>
    <div class="card" style="margin:0">
      <h2>Portfolio Tracker</h2>
      <p>실시간 P&L  +  SL/TP 경보  +  알파 추적</p>
      <div style="margin-bottom:12px;font-size:.84rem;color:{pos_col}">{pos_note}</div>
      <div class="acts">
        <button class="btn pri" onclick="runTracker()">P&L 조회</button>
        <button class="btn ok" onclick="showAddPos()">포지션 추가</button>
        <button class="btn warn" onclick="doRemovePos()">청산</button>
      </div>
      <div id="add-pos" style="display:none;margin-top:13px;border-top:1px solid var(--border);padding-top:13px">
        <div class="fgrid">
          <div class="frow"><label>티커</label><input id="pos-tk" type="text" placeholder="NVDA"></div>
          <div class="frow"><label>수량</label><input id="pos-sh" type="number" min="1" value="10"></div>
          <div class="frow"><label>매수단가 ($)</label><input id="pos-cost" type="number" min="0.01" step="0.01"></div>
          <div class="frow"><label>매수일</label><input id="pos-dt" type="date"></div>
        </div>
        <div class="frow"><label>메모</label><input id="pos-note" type="text" placeholder="예: 1차 매수"></div>
        <div class="acts">
          <button class="btn pri" onclick="doAddPos()">저장</button>
          <button class="btn" onclick="document.getElementById('add-pos').style.display='none'">취소</button>
        </div>
      </div>
      <div class="logbox" id="trk-log"></div>
    </div>
    <div class="card" style="margin:0">
      <h2>Sentiment Scanner</h2>
      <p>뉴스 감성 + 어닝 서프라이즈 + 내부자 거래 + 공매도</p>
      <div class="frow"><label>티커 (비우면 보유 포지션 전체)</label>
        <input id="sent-tk" type="text" placeholder="NVDA, TSLA  또는  공백"></div>
      <div class="acts"><button class="btn pri" id="sent-btn" onclick="runSent()">감성 스캔</button></div>
      <div class="logbox" id="sent-log"></div>
    </div>
  </div>
  <div id="trd-inject" style="padding:0 20px 30px"></div>
</div>
{_JS}
</body></html>"""
...
def main():
    ...
    if server:
        ...
        httpd = _ThreadingHTTPServer(("", PORT), ZeusHandler)
